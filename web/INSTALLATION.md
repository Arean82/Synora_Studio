...
Because Synora Studio is modular, the web portal operates completely independently but **it requires the API Server to be running** to function. Ensure you have completed the `server/INSTALLATION.md` steps first.
...
ExecStart=/path/to/Synora_Studio/venv/bin/python web/web.py
Restart=always
RestartSec=3
...
```bash
pyinstaller --noconfirm --onedir --name "Synora_Web" --add-data "web/templates;templates" --add-data "web/static;static" web/web.py
```
